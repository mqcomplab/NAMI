...
class BitBirch:
    ...
    def fit_with_k_clusters(
        ...
        **gc_kwargs,
        ...
        input_is_packed : bool, default=True
        Whether the input fingerprints are packed
        n_features : int, optional
        Number of features of input fingerprints. Only required for packed inputs if
        ...
        # Set up global clustering algorithm
        if 'random_state' not in gc_kwargs:
            gc_kwargs['random_state'] = random_state
        ...
        print(f"Phase 3: Clustering {len(centroids)} leaf centroids into {n_clusters} final clusters...")
        ...
        #flat_subclusters = [sc for leaf in leaves for sc in leaf._subclusters]
        flat_subclusters=self._get_leaf_bfs()
        ...
        leaf_to_k = {
         ...
         # Create labels array based on number of fitted fingerprints

        self._final_k_labels_ = np.full(self.num_fitted_fps, -1, dtype=int)
    ...
    def get_cluster_sizes(self):
        """
        Get the size (number of molecules) in each final cluster.
        ...
        ------
        ValueError
            If fit_with_k_clusters() has not been called yet.
        """
        labels = self.get_final_labels()
